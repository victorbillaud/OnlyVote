...
module.exports = {
    ...
    verifUserRegistered: (req,res,next) => {
        db.query("SELECT * FROM Users WHERE phoneNumber = ?;", req.headers.phone , function(error, results, fields) {
            if (error) throw error;
            if (results.length == 0) {
                console.log("User not registered")
                res.send({result: false, message: "Vous n'etes pas encore enregistré"});
            }else next()
        })
    },
    register: (req, res, next) => {

        const userData = req.body.socialNumber ? req.body : req.body.data;
        ...
        if(
            ...
            !userData.birthDate ||
            !userData.birthDepartment
        ) res.send({result: false, message: "Informations manquantes"})
